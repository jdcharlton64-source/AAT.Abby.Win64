...
# Build src
bazel build //src/minimal
bazel build //src/abby
bazel build //src/resource_manager
bazel build //src/scheme_handler

# Run examples (Linux/macOS)
bazel run //src/minimal

# Run examples (Windows)
bazel run //src/minimal/win:minimal.exe

# Debug build (add -c dbg flag)
bazel build -c dbg //src/minimal
...
2. **Examples** (`src/`): Demonstration applications showing CEF features
3. **Shared Library** (`src/shared/`): Common functionality used by all examples
...
- **abby**: Shows JavaScript bindings using CefMessageRouter
...
- `src/shared/`: Core shared library with browser utilities
